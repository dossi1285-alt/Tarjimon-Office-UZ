...
echo ===============================================
echo Tarjimon Office UZ - Preflight 1.1.8 single-UAC uninstall test
echo ===============================================
echo.

set "PATCH=%CD%\PATCH_PREFLIGHT_1_1_8_SINGLE_UAC_UNINSTALL.ps1"
...
if not exist "%SOURCE%" (
  echo XATO: Source topilmadi: %SOURCE%
  pause
  exit /b 2
)
if not exist "%BACKUP%" copy /y "%SOURCE%" "%BACKUP%" >nul
if not exist "%BACKUP%" (
  echo XATO: Stabil backup yaratilmadi.
  pause
  exit /b 3
)

echo [1/3] 1.1.8 single-UAC uninstall patch qo'llanmoqda...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%PATCH%"
if errorlevel 1 (
  echo XATO: Patch qo'llanmadi.
  copy /y "%BACKUP%" "%SOURCE%" >nul 2>&1
  pause
  exit /b 4
)
echo OK.
...
if "%RC%"=="0" (
  echo ===============================================
  echo 1.1.8 SINGLE-UAC TEST BUILD MUVAFFAQIYATLI.
  echo ===============================================
) else (
  echo ===============================================
  echo 1.1.8 SINGLE-UAC TEST XATO BILAN TUGADI. Kod: %RC%
  echo ===============================================
)
pause
exit /b %RC%
